...
- "health retest", "re-run the health check", "did my health findings get fixed?" <!-- @rule RL-2daa -->

For a fresh audit, use the `codebase-health` workflow instead.

## Inputs

- The source report sidecar. By default the newest `docs/health/*.json`; the user may name a <!-- @rule RL-ec30 -->
  specific one.
...
- Never invent a new finding in a retest, and never lower a finding's severity. <!-- @rule RL-475c -->
- Match by stable `HL-` id, not by category. <!-- @rule RL-38db -->
- A network-required finding you could not re-check is `needs-manual-verification`, never `fixed`. <!-- @rule RL-d9a7 -->
