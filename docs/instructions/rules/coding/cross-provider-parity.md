...
- Render the host hooks into every hook-capable host's real hook file from one shared renderer in `src/adapters/shared/native-hook-config.ts`, driven by `PAQAD_LIVE_HOOKS` + the per-host `NATIVE_HOOK_EVENTS` map: Claude `.claude/settings.json` and Codex `.codex/hooks.json` render the full pre-and-completion chain (PreToolUse + Stop), Gemini `.gemini/settings.json` `AfterAgent` renders the record-only completion hook. MUST NOT leave it in `ClaudeCodeAdapter` alone or fork a hook script per host — the host is one argv. <!-- @rule RL-93cf -->
...
- Point a completion hook that must never block at the record-only script (`runtime/hooks/verification-record.mjs`) — today only Gemini `AfterAgent`, which requires pure JSON on stdout and cannot accept a blocking Stop decision. A host that documents the same blocking `Stop` semantics as Claude (Codex, issue #566) renders the real `verification-completion.mjs` instead. MUST NOT point a blocking-capable host at the record-only script, nor a JSON-only host at the blocking one. <!-- @rule RL-5616 -->
