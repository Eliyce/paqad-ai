...
- **Two tiers, in this order.** (1) **Provider-native first:** use the host's own <!-- @rule RL-ef6b -->
...
- Render host-triggered behaviour into each host's **native** config, not one <!-- @rule RL-8619 -->
...
- Verify each host's hook surface against that host's own documentation before <!-- @rule RL-96cf -->
...
- Do not close a host-trigger gap by editing the prose entry file <!-- @rule RL-c23e -->
  ...
  the user.
- Point a non-Claude completion hook at the record-only script <!-- @rule RL-a10c -->
  ...
  disrupted.
- When an adapter overrides `generateConfig`/`installHooks` to emit a host config <!-- @rule RL-4396 -->
...
- Prove parity with a test that asserts every hook-wired host emits the hook in <!-- @rule RL-86c7 -->
