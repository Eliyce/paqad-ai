...
- Before compiling the plan, check the Existing surface section and/or run `npx paqad-ai index query <name>`. The plan must record what you checked (`consulted`), what you will reuse (`reusing`), and must justify anything new (`new_constructs`). A plan template without a `reuse` section does not compile, so a change can never quietly rebuild something the project already has. First-party claims are checked against the code-knowledge index (an unknown symbol fails with the nearest match); when a framework is detected, a new construct must also record the framework check, and a framework-native claim must name the installed version. A framework-native claim is additionally verified against the declarations shipped inside the installed package (`npx paqad-ai index framework-api build`): a symbol that does not exist at that version fails with the nearest match, and one carrying a static `@deprecated` marker fails citing the deprecation message — so a plan can never adopt an API that was already removed or is already on its way out. A dynamically-provided member warns rather than failing, and a missing index, stack snapshot, or framework-api index downgrades the check to a warning rather than blocking. <!-- @rule RL-174d -->
- The filled plan template looks like this — `summary` is required, and `reuse` is required: <!-- @rule RL-4a5e -->
...
- Before writing findings, run `npx paqad-ai review digest` and read `.paqad/session/review-digest.md`. It is machine-built from cached evidence in under a second and costs zero model tokens: the changed files and per-stage state, the frozen acceptance criteria, every machine finding (rule scripts, duplication, checks, failing gates), and an honest list of the blind spots no machine covers. <!-- @rule RL-5ddf -->
- Each finding must either reference a digest row or be explicitly marked judgment-only. Confirm or contest every deterministic row — an unaddressed deterministic finding is itself a review finding. The review gate re-derives the deterministic, file-anchored, high-severity rows and **fails** when the recorded review never cites one by `file:line`, so skipping the digest does not disarm the check. <!-- @rule RL-c6e6 -->
