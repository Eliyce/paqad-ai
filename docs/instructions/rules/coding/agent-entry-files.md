...
3. the Adapter: footer — load-bearing, because the per-adapter UI table (which tells
   the host how to surface a decision pause) in the router the bootstrap points to is
   selected by it.
```
...
- An entry file MUST NOT inline a contract (the narration contract, the Decision Pause Contract, or any other); those live in the framework router the bootstrap points to, behind the enablement gate. <!-- @rule RL-487c -->
