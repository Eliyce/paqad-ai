...
- "check my project's health", "codebase health", "code health", "health check", "health audit" <!-- @rule RL-f2f8 -->
- "audit my codebase", "find dead code", "check for unused code/packages", "cleanup audit" <!-- @rule RL-4c78 -->
...
- reuses the code-knowledge index for dead code + unused dependencies (run `paqad-ai index <!-- @rule RL-5785 -->
build` first if it reports `blocked_checks: index-not-built`),
- shells out to `osv-scanner`, `gitleaks`, `jscpd` when they are on PATH and degrades to a <!-- @rule RL-cb61 -->
  labelled fallback or a `blocked_checks` entry (with an install hint) when they are not,
- writes `docs/health/<ts>.md` + `docs/health/<ts>.json` and the per-run finding index. <!-- @rule RL-2b7d -->
...
- Never skip the verb. The findings come from `paqad-ai health run`, never from your own reading. <!-- @rule RL-e398 -->
- If a scanner is unavailable, it is already recorded in `blocked_checks` — surface it, do not work around it. <!-- @rule RL-1af8 -->
- Secret evidence never contains the secret bytes. Only file:line + rule + fingerprint. <!-- @rule RL-6e45 -->
- Deletion, dependency removal, and rewrites are suggestions. Acting on them is the user's call. <!-- @rule RL-7664 -->
- Always keep both the `.md` report and the `.json` sidecar; `health-retest` depends on the sidecar to preserve `HL-` ids. <!-- @rule RL-692c -->
