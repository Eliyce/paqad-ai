...
- Instrument every feature. When you build a user-facing behavior, add the analytics event(s) <!-- @rule RL-331d -->
  for it using the project's detected provider and existing convention. A feature that ships
  untracked is incomplete.
- One convention, 100% consistent. Follow the naming convention already observed in the <!-- @rule RL-239a -->
...
- No variable data in event names. Names are stable identifiers; dynamic values are properties. <!-- @rule RL-37ee -->
- Document each event. Every event has one per-event doc at <!-- @rule RL-43e8 -->
...
- Govern every new event. A brand-new event is a decision, not a silent addition: it must go <!-- @rule RL-e311 -->
...
- Never invent PII capture silently. If an event or property would carry personal data, flag it <!-- @rule RL-bb96 -->
