...
| 3  | **CLAUDE.md resolution**           | Adapter                                                      | `CWD/CLAUDE.md`                                                    | Reads project `CLAUDE.md`. Here it instructs: open `.paqad/framework-path.txt`, resolve `~/.paqad-ai/current`, load rules + stack + design-system                                                                                                                  | Framework entrypoint resolved                     | `CLAUDE.md`, `.paqad/framework-path.txt`, `~/.paqad-ai/current/base/` |
...
| 12 | **Context intelligence / RAG**     | `context-intelligence`, `hybrid-rag`, `cli-rag`              | Workflow + user text                                               | Pulls only the slices it needs: module-map entries, related feature pages, registry rows. Hybrid RAG mixes lexical + semantic. Keeps token budget bounded                                                                                                          | A targeted bundle of file slices                  | `cli-rag`, `hybrid-rag`, `module-map-engine`                          |
| 13 | **Decision Pause check**           | `decision-pause-contract`                                    | Proposed actions                                                   | If the next move is a "pause-worthy" choice (architectural, naming, scope), the agent writes `.paqad/decisions/pending/D-{N}.json` and **stops**. UI surfaces options via `AskUserQuestion`. Execution only resumes once `D-{N}.json` exists in `resolved/`        | Either a resolved decision or a hard stop         | `.paqad/decisions/pending/**`, `.paqad/decisions/resolved/**`         |
| 14 | **Plan / Task graph**              | `cli-plan`, TaskCreate                                       | Resolved decisions + workflow steps                                | For non-trivial work the agent builds a task list (TaskCreate). Each task tracks state independently. For exploratory turns this stage is skipped                                                                                                                  | Ordered task list                                 | task store                                                            |
...
| Decision Pause     | 13     | Agent stops; `.paqad/decisions/pending/D-{N}.json` appears    |
