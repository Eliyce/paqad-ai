# Windsurf — Business View

> Module: **Adapter Onboarding (LLM platform entry files)** (`adapter-onboarding`) · Layer: `cli-commands` · Feature slug: `windsurf`

## Overview

Generates the thin entry file each external LLM agent platform reads on

This page describes **Windsurf** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `windsurf`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `windsurf`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `windsurf` runs.
