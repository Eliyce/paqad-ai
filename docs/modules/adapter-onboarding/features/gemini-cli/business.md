# Gemini CLI (GEMINI.md) — Business View

> Module: **Adapter Onboarding (LLM platform entry files)** (`adapter-onboarding`) · Layer: `cli-commands` · Feature slug: `gemini-cli`

## Overview

Generates the thin entry file each external LLM agent platform reads on

This page describes **Gemini CLI (GEMINI.md)** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `gemini-cli`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `gemini-cli`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `gemini-cli` runs.
