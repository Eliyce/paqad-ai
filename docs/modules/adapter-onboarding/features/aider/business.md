# Aider (config-only) — Business View

> Module: **Adapter Onboarding (LLM platform entry files)** (`adapter-onboarding`) · Layer: `cli-commands` · Feature slug: `aider`

## Overview

Generates the thin entry file each external LLM agent platform reads on

This page describes **Aider (config-only)** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `aider`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `aider`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `aider` runs.
