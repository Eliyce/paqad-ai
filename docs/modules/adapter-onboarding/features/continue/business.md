# Continue — Business View

> Module: **Adapter Onboarding (LLM platform entry files)** (`adapter-onboarding`) · Layer: `cli-commands` · Feature slug: `continue`

## Overview

Generates the thin entry file each external LLM agent platform reads on

This page describes **Continue** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `continue`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `continue`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `continue` runs.
