# Drift Tracking — Business View

> Module: **Stack Detection Engine** (`stack-detection-engine`) · Layer: `framework-internals` · Feature slug: `drift-tracking`

## Overview

Lockfile-first detection of frameworks, traits, and archetypes across

This page describes **Drift Tracking** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `drift-tracking`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `drift-tracking`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `drift-tracking` runs.
