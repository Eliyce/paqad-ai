# paqad-ai module-health — Business View

> Module: **Health & Diagnostics Commands** (`cli-health`) · Layer: `cli-commands` · Feature slug: `module-health`

## Overview

Validate that the framework's generated artifacts, copied instructions,

This page describes **paqad-ai module-health** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `module-health`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `module-health`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `module-health` runs.
