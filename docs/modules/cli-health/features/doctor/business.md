# paqad-ai doctor — Business View

> Module: **Health & Diagnostics Commands** (`cli-health`) · Layer: `cli-commands` · Feature slug: `doctor`

## Overview

Validate that the framework's generated artifacts, copied instructions,

This page describes **paqad-ai doctor** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `doctor`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `doctor`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `doctor` runs.
