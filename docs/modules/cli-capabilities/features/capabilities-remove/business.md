# capabilities remove — Business View

> Module: **Capability Toggle Commands** (`cli-capabilities`) · Layer: `cli-commands` · Feature slug: `capabilities-remove`

## Overview

List and toggle the content / coding / security capability layers with

This page describes **capabilities remove** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `capabilities-remove`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `capabilities-remove`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `capabilities-remove` runs.
