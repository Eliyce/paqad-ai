# capabilities list / available — Business View

> Module: **Capability Toggle Commands** (`cli-capabilities`) · Layer: `cli-commands` · Feature slug: `capabilities-list`

## Overview

List and toggle the content / coding / security capability layers with

This page describes **capabilities list / available** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `capabilities-list`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `capabilities-list`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `capabilities-list` runs.
