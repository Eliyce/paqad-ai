# Custom Pack Authoring — Business View

> Module: **Pack System** (`pack-system`) · Layer: `framework-internals` · Feature slug: `pack-authoring`

## Overview

Declarative `pack.yaml` packs that bind a stack to its detection

This page describes **Custom Pack Authoring** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `pack-authoring`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `pack-authoring`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `pack-authoring` runs.
