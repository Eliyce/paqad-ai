# Override Precedence — Business View
...
This page describes **Override Precedence** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `pack-overrides`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `pack-overrides`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `pack-overrides` runs.
