# Pack Schema (pack.yaml) — Business View

> Module: **Pack System** (`pack-system`) · Layer: `framework-internals` · Feature slug: `pack-schema`

## Overview

Declarative `pack.yaml` packs that bind a stack to its detection

This page describes **Pack Schema (pack.yaml)** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `pack-schema`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `pack-schema`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `pack-schema` runs.
