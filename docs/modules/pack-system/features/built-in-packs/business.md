# 22 Built-in Packs — Business View

> Module: **Pack System** (`pack-system`) · Layer: `framework-internals` · Feature slug: `built-in-packs`

## Overview

Declarative `pack.yaml` packs that bind a stack to its detection

This page describes **22 Built-in Packs** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `built-in-packs`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `built-in-packs`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `built-in-packs` runs.
