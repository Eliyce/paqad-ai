# Differential Refresh — Business View

> Module: **Documentation Workflow** (`documentation-workflow`) · Layer: `agent-workflows` · Feature slug: `differential-refresh`

## Overview

The two-stage flow triggered by "create documentation" (Stage 1 —

This page describes **Differential Refresh** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `differential-refresh`. For `agent-workflows` modules this
typically means:
...
Capture the main journeys for `differential-refresh`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `differential-refresh` runs.
