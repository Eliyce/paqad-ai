# Stage 1 — Foundation Docs — Business View

> Module: **Documentation Workflow** (`documentation-workflow`) · Layer: `agent-workflows` · Feature slug: `foundation-stage`

## Overview

The two-stage flow triggered by "create documentation" (Stage 1 —

This page describes **Stage 1 — Foundation Docs** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `foundation-stage`. For `agent-workflows` modules this
typically means:
...
Capture the main journeys for `foundation-stage`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `foundation-stage` runs.
