# Stage 2 — Module Docs — Business View

> Module: **Documentation Workflow** (`documentation-workflow`) · Layer: `agent-workflows` · Feature slug: `module-stage`

## Overview

The two-stage flow triggered by "create documentation" (Stage 1 —

This page describes **Stage 2 — Module Docs** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `module-stage`. For `agent-workflows` modules this
typically means:
...
Capture the main journeys for `module-stage`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `module-stage` runs.
