...
| --- | --- | --- |
| **planning** | Read canonical docs first; confirm scope; attribute the change to a module | [`scope-check`](../../runtime/base/skills/scope-check/SKILL.md), [`requirement-enrichment`](../../runtime/base/skills/requirement-enrichment/SKILL.md), [`edge-case-detection`](../../runtime/base/skills/edge-case-detection/SKILL.md), the Attribution Gate ([`module-attribution-extractor`](../../runtime/base/skills/module-attribution-extractor/SKILL.md) → [`-inferencer`](../../runtime/base/skills/module-attribution-inferencer/SKILL.md)), [`module-map-reconciler`](../../runtime/base/skills/module-map-reconciler/SKILL.md), [`rule-script-reconciler`](../../runtime/base/skills/rule-script-reconciler/SKILL.md). **Reuse-first (issue #359):** the reuse reflex on every lane, then [`diff-minimizer`](../../runtime/base/skills/diff-minimizer/SKILL.md) + [`existing-doc-checker`](../../runtime/base/skills/existing-doc-checker/SKILL.md) on graduated/full before the plan compiles, and [`cross-module-impact-scanner`](../../runtime/base/skills/cross-module-impact-scanner/SKILL.md) on a multi-module graduated/full change. Agents: [Requirement Analyst](../../runtime/base/agents/requirement-analyst.md), [solution-architect](../../runtime/capabilities/coding/agents/solution-architect.md), [data-modeler](../../runtime/capabilities/coding/agents/data-modeler.md) |
| **specification** | Freeze acceptance criteria and invariants before code (graduated/full need human sign-off) | [`acceptance-criteria-gen`](../../runtime/base/skills/acceptance-criteria-gen/SKILL.md), [`spec-diff`](../../runtime/base/skills/spec-diff/SKILL.md), [`spec-quality-review`](../../runtime/base/skills/spec-quality-review/SKILL.md), [`story-and-solution-writer`](../../runtime/base/skills/story-and-solution-writer/SKILL.md), [`sequence-planner`](../../runtime/base/skills/sequence-planner/SKILL.md), [`test-per-ac-planner`](../../runtime/base/skills/test-per-ac-planner/SKILL.md). Graduated/full add [`performance-regression-estimator`](../../runtime/base/skills/performance-regression-estimator/SKILL.md), [`rollback-safety-planner`](../../runtime/base/skills/rollback-safety-planner/SKILL.md) ([`diff-minimizer`](../../runtime/base/skills/diff-minimizer/SKILL.md) and [`cross-module-impact-scanner`](../../runtime/base/skills/cross-module-impact-scanner/SKILL.md) now run in **planning**, issue #359). When `spec_pipeline_enabled` is on, the stage runs the spec pipeline (`spec pipeline start` through `finish`, with [`expert-notes`](../../runtime/base/skills/expert-notes/SKILL.md) and [`expert-synthesis`](../../runtime/base/skills/expert-synthesis/SKILL.md) when experts are on) and freezes with `spec freeze --from-pipeline` (issue #547). Agents: [Story Designer](../../runtime/base/agents/story-designer.md), [Test Planner](../../runtime/base/agents/test-planner.md), [Product Owner](../../runtime/base/agents/product-owner.md) |
...
| **review** | Digest first (`paqad-ai review digest`, issue #360), then gates, then adversarial + gap passes, then final sign-off | [Verifier](../../runtime/base/agents/verifier.md), then [Adversarial Reviewer](../../runtime/base/agents/adversarial-reviewer.md) + [Gap Detector](../../runtime/base/agents/gap-detector.md) (+ [`adversarial-review`](../../runtime/base/skills/adversarial-review/SKILL.md), [security-auditor](../../runtime/capabilities/security/agents/security-auditor.md), [performance-analyst](../../runtime/capabilities/coding/agents/performance-analyst.md), [ux-ui-analyst](../../runtime/capabilities/coding/agents/ux-ui-analyst.md)), then [Final Reviewer](../../runtime/base/agents/final-reviewer.md). [`test-execution-feedback-loop`](../../runtime/base/skills/test-execution-feedback-loop/SKILL.md) proposes the smallest fix per failing test |
| **checks** | format → test → build, overlapped and with the runner's own parallel mode (issue #554); every parallel-run failure confirmed alone before it blocks; plus module-health and diff-scoped rule scripts | [`rule-script-runner`](../../runtime/base/skills/rule-script-runner/SKILL.md), [`module-health-rollup`](../../runtime/base/skills/module-health-rollup/SKILL.md), [`test-runner-discovery`](../../runtime/base/skills/test-runner-discovery/SKILL.md) (only when `checks plan` reports the runner's parallel mode unknown) |
...
This is the complete roster the framework ships into an onboarded repo: **45 base
...
- [`expert-notes`](../../runtime/base/skills/expert-notes/SKILL.md) — one runner skill with a lens per pickable expert: reads the brief `paqad-ai spec pipeline experts brief <role>` prints and writes request-time findings and questions (issue #547). Knows: the brief, its lens.
- [`expert-synthesis`](../../runtime/base/skills/expert-synthesis/SKILL.md) — the chief architect: accepts or declines each finding, recommends a resolution per conflict, lists gaps, gives a readiness verdict (issue #547). Knows: the merge, the notes, the request, the grounding.

Expert personas (issue #547): the two new pickable experts [QA Engineer](../../runtime/capabilities/coding/agents/qa-engineer.md) and [User Flow Writer](../../runtime/capabilities/coding/agents/user-flow-writer.md), and the [Chief Architect](../../runtime/capabilities/coding/agents/chief-architect.md) who is never picked and runs once any expert fired. They join the nine existing review personas the `expert-notes` lenses adapt for request time.
...
| Reuse-first planning ([`diff-minimizer`](../../runtime/base/skills/diff-minimizer/SKILL.md), [`existing-doc-checker`](../../runtime/base/skills/existing-doc-checker/SKILL.md), [solution-architect](../../runtime/capabilities/coding/agents/solution-architect.md)) | Stops the change from rebuilding what exists and from over-building before code is written | Reuse map + step classification | **Wired in** (issue #359) — named in the enforced planning-stage contract; the reuse reflex fires on every lane, the diff-minimizer/existing-doc-checker procedures on graduated/full |
| [`cross-module-impact-scanner`](../../runtime/base/skills/cross-module-impact-scanner/SKILL.md) | Classifies impact on consuming modules (breaking / silent-shift / additive) | Real consumer-graph reasoning | **Wired in, lane-gated** (issue #359) — named in the enforced planning contract on graduated/full multi-module changes. A fast-lane edit gets none of it |
...
| Review evidence digest (`src/review-digest`) | The hard facts review used to lack: rule findings, duplication, check verdicts, failing gates — plus the blind spots nothing checks | Deterministic, zero model tokens, four cached sources | **Wired in** (issue #360) — named in the enforced review-stage contract, and the review gate fails when a deterministic high-severity row is never cited by file:line |
...
| Stage isolation (`src/stage-isolation`) | Each mandatory stage runs in its own fresh host subagent, with the evidence bundle as the only shared memory, so later stages stop re-carrying earlier stages' history | Structural context economy; token counts recorded per stage agent as `stage-agent` rows in `stage-evidence.jsonl` | **Core engine, lane-gated and enforced** (issues #567, #573) — no config knob; runs on Claude Code and Codex, graduated/full lanes only (the fast lane and non-subagent hosts never dispatch a stage agent). Install **and update** write six `paqad-<stage>` agents at user scope, a record-only `SubagentStop` hook logs each stage agent's `stage-agent` row, and the in-flight fork guard refuses to mint a third bundle on one branch. Each stage subagent proves its own cold framework load via an `agent_id`-keyed entry sentinel. Every stage-evidence row carries the `agent` that wrote it (`orchestrator` or `paqad-<stage>`), and on a graduated/full lane on a subagent-capable host a bundle with no `stage-agent` rows **fails** the completeness gate — so a skipped isolation is machine-detectable, not trusted |
...
reuse-first reflex that fires in planning on every lane, and — since issue #360 — the
review digest, whose deterministic rows the review gate makes non-ignorable. The *richest* cross-module
reasoning (the impact scanner, the full traceability universe scan, RAG) and the
diff-minimizer/existing-doc-checker procedures are **lane-gated or opt-in**, so they
are present but not guaranteed on every edit, and are most absent on fast-lane
changes.
