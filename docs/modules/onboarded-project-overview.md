...
| Stage isolation (`src/stage-isolation`) | Each mandatory stage runs in its own fresh host subagent, with the evidence bundle as the only shared memory, so later stages stop re-carrying earlier stages' history | Structural context economy; token counts recorded per stage agent in `context-efficiency.jsonl` | **Core engine, lane-gated** (issue #567) — no config knob; runs on Claude Code and Codex, graduated/full lanes only (the fast lane and non-subagent hosts never dispatch a stage agent). Install/update always writes six `paqad-<stage>` agents at user scope, a record-only `SubagentStop` hook logs each stage agent's efficiency row, and the in-flight fork guard refuses to mint a third bundle on one branch. Each stage subagent proves its own cold framework load via an `agent_id`-keyed entry sentinel |
