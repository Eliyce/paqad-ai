...
| Stage isolation (`src/stage-isolation`) | Each mandatory stage runs in its own fresh host subagent, with the evidence bundle as the only shared memory, so later stages stop re-carrying earlier stages' history | Structural context economy; token counts recorded per stage agent in `context-efficiency.jsonl` | **Opt-in, lane-gated** (issue #567) — behind the `stage_isolation` flag (default off) on Claude Code and Codex, graduated/full lanes only. When on, install/update writes six `paqad-<stage>` agents at user scope and a record-only `SubagentStop` hook logs each stage agent's efficiency row; when off, every project is byte-identical to before. The fast lane never dispatches a stage agent |
