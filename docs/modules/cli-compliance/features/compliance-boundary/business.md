# compliance boundary — Business View

> Module: **Spec Compliance Commands** (`cli-compliance`) · Layer: `cli-commands` · Feature slug: `compliance-boundary`

## Overview

Extract obligations from structured Markdown specs, check tests for

This page describes **compliance boundary** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `compliance-boundary`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `compliance-boundary`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `compliance-boundary` runs.
