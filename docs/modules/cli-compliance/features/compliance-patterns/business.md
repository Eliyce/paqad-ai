# compliance patterns — Business View

> Module: **Spec Compliance Commands** (`cli-compliance`) · Layer: `cli-commands` · Feature slug: `compliance-patterns`

## Overview

Extract obligations from structured Markdown specs, check tests for

This page describes **compliance patterns** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `compliance-patterns`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `compliance-patterns`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `compliance-patterns` runs.
