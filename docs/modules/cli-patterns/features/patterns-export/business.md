# patterns export — Business View

> Module: **Pattern Library Commands** (`cli-patterns`) · Layer: `cli-commands` · Feature slug: `patterns-export`

## Overview

Query, prune, and export the cross-project pattern library stored at

This page describes **patterns export** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `patterns-export`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `patterns-export`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `patterns-export` runs.
