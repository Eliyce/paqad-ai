...
This page describes **Hybrid Scoring** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `hybrid-scoring`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `hybrid-scoring`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `hybrid-scoring` runs.
