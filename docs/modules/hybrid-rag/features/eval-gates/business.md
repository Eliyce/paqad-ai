# Eval Gates (hit@5, success, correction turns) — Business View
...
This page describes **Eval Gates (hit@5, success, correction turns)** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `eval-gates`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `eval-gates`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `eval-gates` runs.
