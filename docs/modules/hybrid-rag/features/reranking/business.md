# Reranking — Business View

> Module: **Hybrid RAG Runtime** (`hybrid-rag`) · Layer: `framework-internals` · Feature slug: `reranking`

## Overview

Optional vector index under `.paqad/vectors/`, hybrid scoring (vector

This page describes **Reranking** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `reranking`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `reranking`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `reranking` runs.
