# Vector Index — Business View

> Module: **Hybrid RAG Runtime** (`hybrid-rag`) · Layer: `framework-internals` · Feature slug: `vector-index`

## Overview

Optional vector index under `.paqad/vectors/`, hybrid scoring (vector

This page describes **Vector Index** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `vector-index`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `vector-index`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `vector-index` runs.
