# Embedding Providers (local / openai / voyageai) — Business View
...
Document the personas that interact with `embedding-providers`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `embedding-providers`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `embedding-providers` runs.
