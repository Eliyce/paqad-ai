# Semantic Loader — Business View

> Module: **Context Intelligence** (`context-intelligence`) · Layer: `framework-internals` · Feature slug: `semantic-loader`

## Overview

AST-aware semantic loader, 4-tier budget optimizer, deduplication,

This page describes **Semantic Loader** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `semantic-loader`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `semantic-loader`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `semantic-loader` runs.
