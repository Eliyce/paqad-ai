# Turn Summarizer — Business View

> Module: **Context Intelligence** (`context-intelligence`) · Layer: `framework-internals` · Feature slug: `turn-summarizer`

## Overview

AST-aware semantic loader, 4-tier budget optimizer, deduplication,

This page describes **Turn Summarizer** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `turn-summarizer`. For `framework-internals` modules this
typically means:
...
Capture the main journeys for `turn-summarizer`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `turn-summarizer` runs.
