# packs install — Business View

> Module: **Pack Management Commands** (`cli-packs`) · Layer: `cli-commands` · Feature slug: `packs-install`

## Overview

Install, remove, validate, scaffold, and inspect stack packs (built-in,

This page describes **packs install** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `packs-install`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `packs-install`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `packs-install` runs.
