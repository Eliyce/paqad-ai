# packs create — Business View

> Module: **Pack Management Commands** (`cli-packs`) · Layer: `cli-commands` · Feature slug: `packs-create`

## Overview

Install, remove, validate, scaffold, and inspect stack packs (built-in,

This page describes **packs create** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `packs-create`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `packs-create`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `packs-create` runs.
