# Parallel Step Execution — Technical View

> Module: **Feature Development Workflow** (`feature-development-workflow`) · Layer: `agent-workflows` · Feature slug: `parallel-execution`
...
- Triggered by the agent when the user types the workflow's canonical phrase.
- Skill / workflow YAML lives under `runtime/base/skills` or `docs/instructions/workflows/`.

## Data Model / Schema

- Tables, JSON schemas, or YAML schemas owned by this feature.
- For `agent-workflows` modules, this usually means the file shape under `.paqad/` or
...
- Unit test files that cover `parallel-execution` directly.
