# Lane & Complexity Routing — Business View

> Module: **Feature Development Workflow** (`feature-development-workflow`) · Layer: `agent-workflows` · Feature slug: `routing`

## Overview

Generic feature-delivery pipeline assembled from YAML in

This page describes **Lane & Complexity Routing** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `routing`. For `agent-workflows` modules this
typically means:
...
Capture the main journeys for `routing`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `routing` runs.
