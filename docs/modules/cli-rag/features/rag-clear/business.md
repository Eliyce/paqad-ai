# rag clear — Business View

> Module: **RAG Index Commands** (`cli-rag`) · Layer: `cli-commands` · Feature slug: `rag-clear`

## Overview

Build, inspect, clear, and benchmark the optional hybrid RAG vector

This page describes **rag clear** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `rag-clear`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `rag-clear`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `rag-clear` runs.
