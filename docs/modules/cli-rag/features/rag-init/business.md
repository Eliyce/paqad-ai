...
This page describes **rag init** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `rag-init`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `rag-init`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `rag-init` runs.
