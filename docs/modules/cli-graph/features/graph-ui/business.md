# Graph UI (React 19 + Vite SPA) — Business View

> Module: **Project Graph Command** (`cli-graph`) · Layer: `cli-commands` · Feature slug: `graph-ui`

## Overview

One-command WebGL map of modules, files, chunks, symbols, imports, and

This page describes **Graph UI (React 19 + Vite SPA)** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `graph-ui`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `graph-ui`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `graph-ui` runs.
