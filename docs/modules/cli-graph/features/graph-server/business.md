# Graph Server — Business View

> Module: **Project Graph Command** (`cli-graph`) · Layer: `cli-commands` · Feature slug: `graph-server`

## Overview

One-command WebGL map of modules, files, chunks, symbols, imports, and

This page describes **Graph Server** from a business / user-facing perspective.
The technical contract lives at [`technical.md`](./technical.md).

## User Roles

Document the personas that interact with `graph-server`. For `cli-commands` modules this
typically means:
...
Capture the main journeys for `graph-server`. Each flow should call out:
...
- Enumerate invariants that must hold whenever `graph-server` runs.
