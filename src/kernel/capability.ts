...
const ruleScriptsCapability: Capability = {
  ...
  async evaluate({ projectRoot, seam, env, payload }): Promise<CapabilityOutcome> {
    ...
    // the completion seam it means the session OWNS the change and the turn is due its
    // checks (issue #582, the same decision the completion backstop uses). A session that
    // owns nothing, even one a question was misrouted to feature-development for, and an
    // owner on a non-feature detour that edited nothing this turn, run no rule-scripts.
    if (seam === 'pre-mutation' && !payloadFeatureDev(payload, projectRoot)) {
      return NO_OP;
    }
    if (
      seam === 'completion' &&
      !classifyCompletionEnforcement(
        projectRoot,
