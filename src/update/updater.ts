...
export class FrameworkUpdater {
  ...
  async run(projectRoot: string): Promise<UpdateReport> {
    ...
    // in the old folder. Finish it on a later update, once that change has closed.
    if (evidenceMigrationPending(projectRoot)) {
      migrateFeatureEvidence(projectRoot, { sessionId: migrationSessionId() });
    }
