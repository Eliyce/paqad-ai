...
describe('isFrameworkEnabledForRoot (dist-less raw read)', () => {
  ...
  it('reads paqad_enable=false from .paqad/.config', () => {
  ...
  it('does NOT trip on a falsy value under a different key (enterprise)', () => {
    writeConfig(root, 'enterprise=false\n');
    expect(isFrameworkEnabledForRoot(root, NO_ENV)).toBe(true);
  });

  it('stays ON for paqad_enable=true', () => {
...
describe('setFrameworkEnabled', () => {
  ...
  it('flips paqad_enable to false then back to true', () => {
    writeProfile(root, MINIMAL_PROFILE);

    const disabled = setFrameworkEnabled(root, false);
    expect(disabled.enabled).toBe(false);
    expect(isFrameworkEnabledForRoot(root, NO_ENV)).toBe(false);
    expect(readFileSync(disabled.config_path, 'utf8')).toMatch(/^paqad_enable=false$/m);
