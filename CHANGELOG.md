...
- 5c7b0fb: Visual evidence now actually runs from the Stop hook, and a frontend change hears about it at every stage (#579).
  ...
  - **New: `paqad-ai visual-evidence attach <png...> [--ac AC-3] [--label "..."]`.** When a scripted capture cannot run here, attach your own PNG screenshots. They are hashed and recorded as agent-attached (never shown as scripted captures), and a later `visual-evidence run` keeps them. `--ac` must name an acceptance criterion in the frozen spec, so a typo is refused instead of tying screenshots to nothing. A failed scripted step keeps the result partial whichever of `run` and `attach` came first, so attaching screenshots never hides a real gap.
  - **Strict mode is stricter.** Under `visual_evidence_mode=strict`, a frontend change with nothing captured because there is no documented flow or capture script now fails. Attach screenshots, or record a waiver decision resolved with `waive`: the planning readiness decision when one was opened, otherwise one you create with `paqad-ai decision create` (the failing gate prints the exact command). A waived change reads skipped, never pass. Warn mode is unchanged and now prints a "⚪ visual evidence: skipped (reason)" line.
  ...
  - **On the record.** The late gates (bundle-completeness, visual-evidence, rules-loaded) are now written to the bundle's `evidence.jsonl` when the evidence ledger is on, skips included, with a new `skipped` verdict that never counts as a pass or a fail. `audit export` grades a skipped row informational (OCSF severity 1, CEF 1), and the evidence packet shows it with a grey dot instead of the amber "needs a look". A passing gate now names the `(proof: visual)` criteria it covers.

## 1.90.1

### Patch Changes

- c72b67b: fix(#576): make `paqad-ai join` fully provision a teammate machine and stop the first prompt of a session from falsely blocking at Stop
...
- 6791fef: fix(#573): build the prompt-route seam so a change gets a real lane, and enforce stage isolation instead of trusting it
...
- e6c3737: feat(#567): run each feature-development stage in a fresh subagent, with the evidence bundle as shared memory
  ...
  records under the orchestrator's session id, so the bundle keeps one identity — and each stage
  subagent proves its own cold framework load through an `agent_id`-keyed entry sentinel (a
  subagent shares its parent's `session_id`, so the sentinel keys on the distinct `agent_id`).
...
- 1952ac7: feat(codex-cli): wire paqad's full hook set so feature-development stages are enforced on Codex exactly as on Claude Code (#566)
...
- 533cf1b: Require and evidence rule-loading for feature-development changes (#557).
...
- 13efb31: Make the feature-development `checks` stage fast without lowering the bar (#554, closes #505).
...
- e5e56fc: feat(#551): PR visual evidence — screenshots of documented flows as feature-bundle evidence
...
- c6f611c: feat(#547): wire the spec pipeline and ship the domain experts
...
- 912d9e3: fix(#540): stop a background notification opening a change and orphaning a bundle
...
- 553c6db: feat(#538): keep the coding agent's vendor out of your commits and PRs
...
- 19afcc6: fix(#528): re-home the checks report into the per-feature evidence bundle
...
- 1e369b2: feat(spec-pipeline): model-driven expert roster, off by default (issue #521, Phase 2)
...
- fd71d26: feat(spec-pipeline): requirement-enrichment emits FR-4 two-layer question objects (B.5.2)
...
- 1055da9: spec-pipeline: S0 grounding now has a RAG-on path (FR-2.1, deferred from #512). A new
...
- 1055da9: spec-pipeline: the S4 crafting skill now reuses `edge-case-detection` to populate negative
...
- 9216d33: Spec pipeline (#517): wire the deferred S2 ledger auto-answer step (FR-4.5). Before any
...
- 3067adb: Feature bundles are now provably complete. A finished feature-development change must leave every required file in its evidence bundle, and a missing one now fails the change instead of passing silently (issue #511):
...
- 018aac8: Spec pipeline (#512), Part A: every `spec freeze` now writes a human-readable
...
- dce2fc6: Rebuild the site-map workflow so it earns the trust it reports. The engine drafts the map
...
- adbf422: Add the code-knowledge index (issue #353): a deterministic, offline, zero-LLM map of every exported symbol (name, kind, file:line, signature, caller count, orphan flag) and the import/reference edges between files, persisted to `.paqad/indexes/code-knowledge.json`.
...
- c900ac0: Per-feature evidence bundle (#339): re-key the stage-evidence spine onto one
...
- fd49d8b: Per-feature evidence bundle — Phase 1 foundation (dark) (#339).
...
- bd42a6a: Route every prompt to one of 9 workflows first; load rules only for feature-development (#336).
...
- 30534cf: Add the complementary analytics-instrumentation agent (#241): an opt-in, coding-first,
...
- 7d48ba0: Ship a `decision` skill that authors Decision Pause packets — mint collision-free `D-<ULID>` ids, drive the pending → resolved lifecycle, and commit resolved packets with the PR (issue #272).
...
- 46e8d7a: Make the mandatory `review` stage recordable when review happens after tests/docs (issue #270).

  The stage-evidence ledger enforces a canonical stage order (`planning → specification → development → review → checks → documentation_sync`) and rejects an earlier stage that starts after a later one, so the recorded order can't be faked. `review` is the one stage this punished unfairly: it is **edit-less** (a review of the finished diff mutates no file, so the live writer can never stamp it) yet canonically **precedes** `checks` and `documentation_sync` — the stages the live writer _does_ stamp from the tests and docs written during the build. An honest review of the completed change therefore lands, in wall-clock time, after those rows already exist, and the completeness gate reported it as `incomplete — missing [review]`, indistinguishable from a skipped review.
...
- ab9db21: Stage-evidence enforcement: a deterministic gate that fails a code change whose feature-development workflow was left incomplete.
...
- da75225: Cross-platform hooks, always-load entry gate, and two new evidence ledgers.
...
- [#233](https://github.com/Eliyce/paqad-ai/pull/233) [`8e667c7`](https://github.com/Eliyce/paqad-ai/commit/8e667c7d81954735f2c682f978d2a1e7b60da20f) Thanks [@HLasani](https://github.com/HLasani)! - Lean entry files: load the framework and contracts only when paqad is enabled ([#229](https://github.com/Eliyce/paqad-ai/issues/229))
...
- [#228](https://github.com/Eliyce/paqad-ai/pull/228) [`8522104`](https://github.com/Eliyce/paqad-ai/commit/8522104679eb304e8e622ded5f773f0f9281d787) Thanks [@HLasani](https://github.com/HLasani)! - Move framework configuration out of `project-profile.yaml` into a Laravel-style four-surface config layer.
...
- [#223](https://github.com/Eliyce/paqad-ai/pull/223) [`4ce2295`](https://github.com/Eliyce/paqad-ai/commit/4ce22959b54f148885dfea6c6580a375612f8b23) Thanks [@HLasani](https://github.com/HLasani)! - Preserve user config on re-onboard, always show the enable switch, and stop the dashboard in vanilla mode ([#220](https://github.com/Eliyce/paqad-ai/issues/220) follow-up)
...
- [#188](https://github.com/Eliyce/paqad-ai/pull/188) [`690ce1e`](https://github.com/Eliyce/paqad-ai/commit/690ce1e80f44c95c7027f8af2df973c9734de11c) Thanks [@HLasani](https://github.com/HLasani)! - Gate the evidence ledger behind an opt-in `enterprise` config block (disabled by default).
...
- [#185](https://github.com/Eliyce/paqad-ai/pull/185) [`2bec5ec`](https://github.com/Eliyce/paqad-ai/commit/2bec5eca66ec4aad7b9541d5ddd99f538102b6df) Thanks [@HLasani](https://github.com/HLasani)! - Make onboarded repos team-safe so parallel branches no longer collide.
...
- [#156](https://github.com/Eliyce/paqad-ai/pull/156) [`6cf600c`](https://github.com/Eliyce/paqad-ai/commit/6cf600cc36888edd6eeae66eb103a3e620372e50) Thanks [@HLasani](https://github.com/HLasani)! - Complete the verifiable-trust receipt with two optional, additive fields ([#122](https://github.com/Eliyce/paqad-ai/issues/122),
  [#123](https://github.com/Eliyce/paqad-ai/issues/123)). Both fold into the existing [#118](https://github.com/Eliyce/paqad-ai/issues/118) receipt + [#146](https://github.com/Eliyce/paqad-ai/issues/146) Trust dashboard the same
  way [#120](https://github.com/Eliyce/paqad-ai/issues/120) authorship did, and are omitted when absent so receipts for projects
  without them stay byte-identical.

  **[#122](https://github.com/Eliyce/paqad-ai/issues/122) — gate→clause compliance citations.** A new `compliance-pack` kind (its
  ...
  **[#123](https://github.com/Eliyce/paqad-ai/issues/123) — reproducibility stamp.** Promote paqad's deterministic context rebuild
...
- [#154](https://github.com/Eliyce/paqad-ai/pull/154) [`a709140`](https://github.com/Eliyce/paqad-ai/commit/a70914026595dbb6f6adc20a174bbbed706d62ea) Thanks [@HLasani](https://github.com/HLasani)! - Add `paqad-ai audit export` — a read-only, local-first exporter that projects the
  unified evidence ledger and tamper-evident receipt chain ([#118](https://github.com/Eliyce/paqad-ai/issues/118)/[#120](https://github.com/Eliyce/paqad-ai/issues/120)) into the
...
- [#142](https://github.com/Eliyce/paqad-ai/pull/142) [`4dbc2b2`](https://github.com/Eliyce/paqad-ai/commit/4dbc2b2e349ff2bf13b56627aab08cb7e955f807) Thanks [@HLasani](https://github.com/HLasani)! - Fix and enforce silent self-update so onboarded installs actually stay current.
...
- [#138](https://github.com/Eliyce/paqad-ai/pull/138) [`2cfd738`](https://github.com/Eliyce/paqad-ai/commit/2cfd7387edc70b87543db65a02e49e5144b9a3b7) Thanks [@HLasani](https://github.com/HLasani)! - `paqad-ai evidence` — render verification evidence as a scannable PR comment ([#119](https://github.com/Eliyce/paqad-ai/issues/119)).
  ...
  - The footer honestly attests "paqad's gates passed for this run, not that the
    ...
    from [#118](https://github.com/Eliyce/paqad-ai/issues/118), not a replacement for it.
  ...
  **Auto-post for every onboarded project.** The same render is now posted to
  the PR automatically by the delivery CI gate ([#42](https://github.com/Eliyce/paqad-ai/issues/42)), so the proof lands without
