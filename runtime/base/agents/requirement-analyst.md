...
Freeze runs the spec-quality review itself (issue #401), so never run `compliance review` by hand to satisfy the defect clause above. An open **critical** defect prints as a freeze blocker like any other and the command exits non-zero; major and minor findings never block. The defect summary is folded into `specification.json`, so the feature-development flow produces no `.paqad/compliance/<slug>/spec-review.json` — the standalone `compliance review` verb remains available for explicit compliance work, but freeze never requires it. Author the spec markdown **inside the project** (anywhere outside a feature bundle dir): a path resolving outside the project root is rejected, because the frozen record pins a project-relative path that has to read the same on every machine.
