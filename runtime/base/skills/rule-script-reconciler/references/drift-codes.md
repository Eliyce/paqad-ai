...
| `RS-CONFLICT`      | Two rules contradict each other. Emitted by the **rule-analyzer's** semantic pass — not the deterministic reconciler, which cannot decide contradiction. Recorded into drift.json so the vocabulary is complete. | `analyze rules` (re-runs the conflict pass) then resolve via the Decision Pause Contract. |
