# Astro Testing Guide

Use the narrowest command that proves the changed behavior, then expand to the full gate before merge.

## Common Commands

- focused test: `pnpm test -- <path-or-pattern>`
- full suite: `pnpm test`
- lint: `pnpm lint`
- typecheck: `pnpm run typecheck`
- build check: `pnpm build`
- if Docker Compose is active, prefix with `docker compose exec <node-service>`

## Coverage Expectations

- cover happy path plus 404 and content-rendering error states that changed
- include end-to-end tests for dynamic routes and API endpoints using the Astro test utilities
